...
4. **Reviewer Heuristics:** To provide actionable feedback without requiring strict JSON structural validation from the external API, the Orchestrator appends a strict "numbered list" constraint to the Reviewer's prompt. It then reuses an internal `_parse_tasks_from_text` static method to extract only the high-level refinements (ignoring the LLM's supporting text) and maps them to a `ReviewerStep` dataclass list. Streamlit then elegantly uses `len(tasks)` to dynamically update the UI with the exact number of suggestions.
